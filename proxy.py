...
class ProxyHandler(BaseHTTPRequestHandler):
    ...
    def log_message(self, fmt, *args):
        print(f"[proxy] {self.headers.get('CF-Connecting-IP', self.client_address[0])} {fmt % args}")

if __name__ == "__main__":
    listen_port = int(os.environ.get("PROXY_PORT", "7900"))
    listen_host = os.environ.get("PROXY_HOST", "127.0.0.1")
    server = HTTPServer((listen_host, listen_port), ProxyHandler)
    print(f"IP whitelist proxy running on {listen_host}:{listen_port} -> {UPSTREAM}")
